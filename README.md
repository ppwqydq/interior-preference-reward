...
1. SA-IQA 和 Qwen3-VL 都能在内部数据上学习到部分用户偏好规律。
2. 固定验证集结果明显高于 OOF 和独立外部测试，存在乐观偏差。
3. 房型会形成标签捷径，不能继续作为正式输入特征。
4. Margin-L2 没有带来稳定的外部泛化收益。
5. 当前主参考模型仍为无房型 Qwen3-VL Epoch 8。
6. 当前主要瓶颈是数据覆盖、数据划分、标签主观性和训练目标与 Pairwise 使用目标不完全一致。
7. 下一阶段将使用约五千多组“用户原图 + 最终生成图”双图数据，重点提升独立新数据上的泛化能力。